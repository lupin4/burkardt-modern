...
contains
  ...
  !    Input, integer(int32) ELEMENT_NUM, the number of elements.
  !
  !    Input, real(real64) NODE_XY(2,NODE_NUM), the node coordinates.
  !
  !    Input, integer(int32) ELEMENT_NODE(4,ELEMENT_NUM), lists the 
  !    nodes that make up each element, in counterclockwise order.
  !
  !    Output, real(real64) ELEMENT_AREA(ELEMENT_NUM), the element areas.
  !
  !    Output, real(real64) MESH_AREA, the mesh area.
  !
    ...
    integer(int32) element
    real(real64) element_area(element_num)
    integer(int32) element_node(4,element_num)
    real(real64) mesh_area
    integer(int32) node
  ...
  !  Parameters:
  !
  !    Input, real(real64) QUAD_XY(2,4), the coordinates of the nodes.
  !
  ...
  !
    ...
    if ( area1 < 0.0e+00_real64 .or. area2 < 0.0e+00_real64 ) then
  ...
  !
    ...
    integer(int32) e1(4*element_num)
    integer(int32) e2(4*element_num)
    integer(int32) edge(2,4*element_num)
  ...
  !    nodes.
  !
  !    Output, integer(int32) ELEMENT_NODE(4,ELEMENT_NUM), the nodes
  !    that make up the elements.
  !
  !    Output, integer(int32) ELEMENT_NEIGHBOR(4,ELEMENT_NUM), the 
  ...
  !
    ...
    node_xy = reshape ( (/ &
         ...
         4.0e+00_real64, 4.0e+00_real64 /), (/ dim_num, node_num /) )
  ...
  !
    ...
    k = 1
    node_xy(1,k) = 0.0e+00_real64
    node_xy(2,k) = 0.0e+00_real64

    do row = 1, 4
      r = real ( row, real64)
      do col = 0, 8
        a = real ( 8 - col, real64) * pi / 8.0e+00_real64
        k = k + 1
        node_xy(1,k) = r * cos ( a )
        node_xy(2,k) = r * sin ( a )
      end do
  ...
  !    Input, integer(int32) ELEMENT_NUM, the number of elements.
  !
  !    Input, integer(int32) ELEMENT_NODE(4,ELEMENT_NUM), 
  ...
  !
    if ( 2 <= element_show ) then
      ...
      do element = 1, element_num
        ...
        ave_x = ave_x / real ( element_order, real64)
        ave_y = ave_y / real ( element_order, real64)
  ...
  !  Parameters:
  !
  !    Input, real(real64) Q4(2,4), the coordinates of the vertices.
  ...
  !
  !    Input, real(real64) RS(2,N), (R,S) points in the reference element.
  !
  !    Output, real(real64) XY(2,N), (X,Y) points in the physical element.
  !
    ...
    psi(1,1:n) = ( 1.0e+00_real64 - rs(1,1:n) ) * ( 1.0e+00_real64 - rs(2,1:n) )
    psi(2,1:n) =             rs(1,1:n)   * ( 1.0e+00_real64 - rs(2,1:n) )
    psi(3,1:n) =             rs(1,1:n)   *             rs(2,1:n)
    psi(4,1:n) = ( 1.0e+00_real64 - rs(1,1:n) ) *             rs(2,1:n)
  ...
  !    that form the elements.
  !
  !    Input, integer(int32) SAMPLE_NUM, the number of points to sample.
  !
  !    Input/output, integer(int32) SEED, a seed for the random 
  !     number generator.
  !
  !    Output, real(real64) SAMPLE_XY(2,SAMPLE_NUM), the sample points.
  !
  !    Output, integer(int32) SAMPLE_ELEMENT(SAMPLE_NUM), the elements from
  ...
  !
    ...
    integer(int32) sample_element(sample_num)
    real(real64) sample_xy(2,sample_num)
    integer(int32) seed
  !
  ...
  !     number generator.
  !
  !    Output, real(real64) XY(2,N), the sample points.
  !
