...
contains
  ...
  pure subroutine sphere_cubed_grid_ijk_to_xyz ( n, i, j, k, xyz ) &
        bind(C, name="sphere_cubed_grid_ijk_to_xyz")
  ...
  !
    ...
    if ( i == 0 ) then
      ...
      xc = tan ( real ( 2 * i - n, dp) * 0.25_dp * r8_pi &
        / real ( n, dp) )
    end if
    ...
    if ( j == 0 ) then
      ...
      yc = tan ( real ( 2 * j - n, dp) * 0.25_dp * r8_pi &
        / real ( n, dp) )
    end if
    ...
    if ( k == 0 ) then
      ...
      zc = tan ( real ( 2 * k - n, dp) * 0.25_dp * r8_pi &
        / real ( n, dp) )
    end if
    ...
    xyz(2) = yc / xyzn
    xyz(3) = zc / xyzn
  end subroutine sphere_cubed_grid_ijk_to_xyz

  pure subroutine sphere_cubed_grid_line_count ( n, line_num ) &
        bind(C, name="sphere_cubed_grid_line_count")
  ...
  end subroutine sphere_cubed_grid_line_count

  subroutine sphere_cubed_grid_lines ( n, line_num, line_data ) &
        bind(C, name="sphere_cubed_grid_lines")
  ...
  !
  !    Input, integer(ip) LINE_NUM, the number of lines.
  !
  !    Output, real(dp) LINE_DATA(3,2,LINE_NUM), for each line I, the 
  ...
  !
    ...
    integer(ip) :: j
    integer(ip) :: l
    real(dp), intent(out) :: line_data(3,2,line_num)
    integer(ip), intent(in), value :: n
  ...
  end subroutine sphere_cubed_grid_lines

  subroutine sphere_cubed_grid_lines_display ( line_num, line_data, prefix ) &
        bind(C, name="sphere_cubed_grid_lines_display")
  ...
  !
  !    Input, real(dp) LINE_DATA(3,2,LINE_NUM), contains pairs of 
  ...
  !
    ...
    integer(ip) :: j
    integer(ip) :: l
    real(dp), intent(in) :: line_data(3,2,line_num)
  ...
  pure subroutine sphere_cubed_grid_point_count ( n, ns ) &
        bind(C, name="sphere_cubed_grid_point_count")
  ...
  !
    ...
    ns = ( n + 1 ) ** 3 - ( n - 1 ) ** 3
  end subroutine sphere_cubed_grid_point_count

  subroutine sphere_cubed_grid_points ( n, ns, xyz ) &
        bind(C, name="sphere_cubed_grid_points")
  ...
  end subroutine sphere_cubed_grid_points

  subroutine sphere_cubed_grid_points_display ( ng, xg, prefix ) &
        bind(C, name="sphere_cubed_grid_points_display")
  ...
  !
    ...
    write ( *, '(a)' ) &
      '  Created command file "' // trim ( command_filename ) // '".'
  end subroutine sphere_cubed_grid_points_display

  subroutine sphere_cubed_grid_points_face ( n, i1, j1, k1, i2, j2, k2, ns, xyz ) &
        bind(C, name="sphere_cubed_grid_points_face")
  ...
  end subroutine sphere_cubed_grid_points_face
