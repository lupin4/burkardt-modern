...
!  Parameters:
!
!    Input, double precision V1(3), V2(3), V3(3), the Cartesian coordinates
!    of unit vectors (the three triangle vertices in any order).  These 
!    vectors, if nonzero, are implicitly scaled to have length 1.
!
!    Output, double precision AREAS, the area of the spherical triangle 
...
!  Parameters:
!
!    Input, double precision V1(3), V2(3), V3(3), the coordinates of the 
...
!    deletion.
!
!    Input/output, double precision X(N), Y(N), Z(N), the coordinates of 
...
!
!    Output, double precision DF, value of an increasing function (negative
...
!  Parameters:
!
!    Input, double precision P(3), the coordinates of the point (unit vector)
!    to be located.
!
!    Input, integer LV, the length of arrays XV, YV, and ZV.
!
!    Input, double precision XV(LV), YV(LV), ZV(LV), the coordinates of unit
...
!
!    Input, double precision CN(3), the coordinates of a nonzero vector 
...
!
!    Output, double precision P(3), point of intersection defined above 
...
!
!    Output, double precision PLAT, the latitude of P in the range -PI/2 
!    to PI/2, or 0 if PNRM = 0.
!
!    Output, double precision PLON, the longitude of P in the range -PI to PI,
...
!
!    Output, double precision STORE, the value of X after it has been stored
...
!
!    Output, double precision X(N), Y(N), Z(N), the coordinates in the 
...
!
!    Input, double precision P(3), the x, y, and z coordinates (in that order)
!    of the point P to be located.
!
!    Input, integer N, the number of nodes in the triangulation.
!    3 <= N.
!
!    Input, double precision X(N), Y(N), Z(N), the coordinates of the
!    triangulation nodes (unit vectors).
!
!    Input, integer LIST(6*(N-2)), LPTR(6*(N-2)), LEND(N), the 
!    data structure defining the triangulation, created by TRMESH.
!
!    Output, double precision B1, B2, B3, the unnormalized barycentric
...
!    unprocessed node for use by ADDNOD.
!
!    Workspace, double precision DIST(N), 
!    used to efficiently determine the nearest triangulation node to each
!    unprocessed node for use by ADDNOD.
!
!    Output, integer IER, error indicator:
...
!
!    Input, double precision PLTSIZ, the plot size in inches.  A circular
...
!
!    Input, double precision ELAT, ELON, the latitude and longitude 
...
!    180, respectively.
!
!    Input, double precision A, the angular distance in degrees from E 
...
!
!    Input, double precision ELAT, ELON, the latitude and longitude (in
...
!
!    Input, double precision A, the angular distance in degrees from E to the
...
!    K for K = 1 to N.
!
!    Input, double precision XC(NT), YC(NT), ZC(NT), the coordinates of the
