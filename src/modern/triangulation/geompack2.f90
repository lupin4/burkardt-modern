...
!  Parameters:
!
!    Input/output, integer NVC, the number of coordinates or 
!    positions used in VCL array.
!
!    Input, integer NPOLG, the number of polygons or positions used 
!    in HVL array.
!
!    Input, integer NVERT, the number of vertices or positions 
!    used in PVL array.
!
!    Input, integer MAXVC, the maximum size available for 
!    VCL array.
!
...
!
!    Output, integer VSTART(1:NVERT), the start location in VCL 
!    for mesh vertices on each edge in PVL if there are any, else 0.
!
!    Output, integer VNUM(1:NVERT), the number of mesh vertices 
...
!
!    Input/output, integer MAP(N); on input, the points of A with
...
!
!    Input/output, integer PVL(1:4,1:NVERT), double precision !    IANG(1:NVERT), the polygon vertex list and interior angles; see routine 
...
!
!    Input, integer LDV, the leading dimension of VCL in calling 
!    routine.
!
!    Input, integer NT, the number of triangles in strip or 
!    polygon.
!
!    Input, VCL(1:2,1:*), the vertex coordinate list.
!
!    Input/output, integer TIL(1:3,1:NT), the triangle incidence
!    list.
!
!    Input/output, integer TEDG(1:3,1:NT) - TEDG(J,I) refers to 
...
!
!    Input, integer NODTRI(3,NUM_TRI), the nodes that make up 
!    the triangles.
!
!    Input, integer TNBR(3,NUM_TRI), the triangle neighbors on 
!    each side.
!
  integer num_pts
  integer num_tri
...
!    Input, integer N, the number of points.
!
!    Input, integer LDA, the leading dimension of array A in the 
...
!  Parameters:
!
!    Input, double precision X0, Y0, X1, Y1, X2, Y2, X3, Y3, the points.
!
...
!
!    Output, integer I1, I2 , indices in XC, YC of the diameter 
...
!  Parameters:
!
!    Input, integer L, U, the lower and upper indices of part of 
!    the heap.
!
!    Input, integer K, the spatial dimension of the points.
!
!    Input, integer LDA, the leading dimension of A in the calling
!    routine.
!
!    Input, double precision A(LDA,N); A(I,J) contains the I-th coordinate
!    of point J.
...
!
!    Input, integer NVBC(1:NHOLE+1), the number of vertices per 
...
!
!    Input, integer MAXHO, the maximum size available for HOLV 
!    array; should be greater than or equal to NHOLE*2.
!
!    Output, integer NVC, the number of vertex coordinates,
!    set to sum of NVBC(I).
!
!    Output, integer NPOLG, the number of polygonal subregions, 
!    set to 1.  For consistency with DSPGDC.
!
!    Output, integer NVERT, the number of vertices in PVL, set to
!    NVC.  For consistency with DSPGDC.
!
!    Output, integer NHOLA, number of attached holes, set to 0.
!    For consistency with DSPGDC.
!
!    Output, integer REGNUM(1:1), region number of only subregion,
!    set to 1.  For consistency with DSPGDC.
!
!    Output, integer HVL(1:NHOLE+1), the head vertex list; first 
!    entry is the head vertex (index in PVL) of outer boundary curve; next
!    NHOLE entries contain the head vertex of a hole.
!
!    Output, integer PVL(1:4,1:NVC), IANG(1:NVC), the polygon 
...
!
!    Output, integer HOLV(1:NHOLE*2), the indices in PVL of top 
...
!
!    Input, integer NVC, the number of vertex coordinates in VCL 
!    array.
!
!    Input, integer NPOLG, the number of polygonal subregions in 
!    HVL array.
!
!    Input, integer MAXWK, the maximum size available for WK 
...
!    Input, integer HVL(1:NPOLG), the head vertex list.
!
!    Input, integer PVL(1:4,1:*), double precision IANG(1:*), the 
!    polygon vertex list, interior angles.
!
!    Output, integer IVRT(1:*), the indices of polygon vertices in
...
!  Parameters:
!
!    Output, double precision TIME, the current reading of the CPU clock
!    in seconds.
!
  integer clock_count
  integer clock_max
...
!
!    Input/output, integer NVC, the number of positions used 
!    in VCL array.
!
!    Input/output, integer NVERT, the number of positions used in PVL array.
!
...
!    maintained in heap.
!
!    Input/output, integer LOCH(1:*), the location of indices in
...
!  Parameters:
!
!    Input, integer NVRT, the number of vertices on the boundary
!    of convex polygon.
!
!    Input, double precision XC(0:NVRT), YC(0:NVRT), the vertex coordinates 
!    in counter clockwise order;
...
!    distances for edges.
!
!    Output, integer NSHR, the number of vertices on boundary of
...
!
!    Output, integer IEDGE(0:NVRT), the indices of edges of 
...
!
!    Input, integer NHOLA, the number of 'attached' holes; these
...
!    array; should be about 5 times maximum number of vertices in any polygon.
!
!    Input, integer HOLV(1:NHOLE*2+NHOLA), the indices in PVL of 
...
!    Input/output, integer HVL(1:NPOLG), the head vertex list.
!
!    Input/output, integer PVL(1:4,1:NVERT), 
!    double precision IANG(1:NVERT), the polygon vertex list and interior 
...
!
!    Input, integer MAXST, the maximum size available for the 
!    STACK array.
!
...
!
!    Input, integer NBL, the number of vertices on boundary 
!    cycle if INTER, otherwise on left boundary chain.
!
!    Input, integer NCR, the number of vertices on closed walk 
!    if INTER, otherwise on right boundary chain.
!
!    Input, integer CHBL(0:NBL), the indices in VCL of vertices
!    on boundary cycle or left boundary chain; if INTER, CHBL(NBL) = CHBL(0).
!
!    Input, integer CHCR(0:NCR), the indices in VCL of vertices 
!    on closed walk or right boundary chain; if INTER, CHCR(NCR) = CHCR(0),
!    otherwise CHCR(0) is not referenced.
!
!    Input, integer LDV, the leading dimension of VCL in 
!    calling routine.
!
!    Input, double precision VCL(1:2,1:*), the vertex coordinate list.
!
!    Output, integer TIL(1:3,1:NT), the triangle incidence list,
!    where NT = NBL + NCR - K where K = 0 if INTER, else K = 2.
!
!    Output, integer TEDG(1:3,1:NT), the TEDG(J,I) refers to 
!    edge with vertices TIL(J:J+1,I) and contains index of merge edge or 
...
!
!    Input, double precision G_XY(2,G_NUM), the coordinates of the points.
!
...
!    the convex polygons.
!
!    Output, double precision H(1:NPOLG), the triangle size for
!    convex polygons.
!
...
!
  ...
  integer , save :: m2 = 0
  integer , save :: mic = 0
  double precision , save :: s = 0.0D+00
  double precision urand
!
...
!  Parameters:
!
!    Input, double precision X, Y, the coordinates of a 2D point outside
!    the convex hull.
!
!    Input, double precision VCL(1:2,1:*), the coordinates of 2D vertices.
!
...
!  Parameters:
!
!    Input, double precision XEYE, YEYE, the coordinates of the eyepoint.
!
...
!  Parameters:
!
!    Input, double precision X, Y, the coordinates of a 2D point.
!
!    Input, integer NTRI, the number of triangles in the triangulation; used 
!    to detect cycle.
!
!    Input, double precision VCL(2,1:*), the coordinates of 2D vertices.
!
