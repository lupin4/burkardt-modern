...
!  Parameters:
!
!    Input, double precision U(1:3), V(1:3), the vectors.
!
!    Input, double precision RTOLSQ, the relative tolerance used to 
!    detect a zero vector, based on the square of the Euclidean length.
!
!    Output, double precision ANGLE3, the angle between the two vectors,
...
!    polygon.  3 <= NVRT.
!
!    Input, double precision XC(1:NVRT), YC(1:NVRT), the vertex coordinates
...
!    records in FC.
!
!    Input/output, integer FC_NUM, current number of records used
!    in FC.
!
...
!    records in FC.
!
!    Input/output, integer NFC, current number of records used 
!    in FC.
!
!    Input, integer FC_MAX, maximum number of records available 
!    in FC.
!
!    Input, integer FC(1:K+4,1:*), array of face records; see 
!    routine DTRISK.
!
!    Output, integer POS, position of available record (if FC 
!    not full).
!
!    Output, integer IERR, error flag, which is zero unless an 
!    error occurred.
...
!
!    Input, integer IND(1:K+1), indices in VCL of K-D vertices 
!    of simplex.
!
!    Input, double precision VCL(1:K,1:*), K-D vertex coordinate list.
!
!    Input, double precision PT(1:K), K-D point for which barycentric
!    coordinates are to be computed.
!
!    Output, double precision ALPHA(1:K+1), barycentric coordinates 
...
!    degenerate simplex.
!
!    Workspace, double precision MAT(1:K,1:K), matrix used for solving 
...
!  Parameters:
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), 4 vertices
!    of tetrahedron.
!
!    Input, double precision E(1:3), fifth point for which 
!    barycentric coordinates found
!
!    Output, double precision ALPHA(1:4), the scaled barycentric coordinates
!    (if DEGEN = .FALSE.) such that 
...
!
!    Input, integer BF_NUM, the number of boundary faces or 
!    triangles.
!
!    Input, integer NBPT, the number of vertices (points) on 
!    boundary of convex hull.
!
!    Input, integer NIPT, the number of vertices (points) in 
!    interior of convex hull.
!
!    Input, integer SIZHT, the size of hash table HT; a good 
...
!
!    Input, double precision VCL(1:3,1:*), the vertex coordinate list.
!
...
!  Parameters:
!
!    Input, double precision BINEXP, the exponent for number of bins.
!
!    Input, integer N, the number of points.
!
!    Input, double precision A(1:2,1:*), array of points.
!
!    Input/output, integer MAP(1:N).  On input, the points of 
...
!    Input, integer N, the number of points.
!
!    Input, double precision A(1:3,1:*), the array of points.
!
...
!    Input, integer N, the number of points.
!
!    Input, double precision A(1:K,1:*), array of points.
!
!    Input/output, integer MAP(1:N).  On input, the points of A 
...
!    Workspace, integer IWK(1:N), used for copy of MAP array.
!
!    Workspace, double precision DX(1:K), used for size of range of
!    coordinates.
!
  implicit none
...
!  Parameters:
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), the 
!    vertices of the tetrahedron.
!
!    Output, double precision CCRADI, the value 1/(4*circumradius)**2.
!
  implicit none
...
!    to be made.
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), vertices 
!    of tetrahedron.
!
!    Input, double precision E(1:3), a fifth point; referenced iff 
!    INTEST is TRUE.
!
!    Output, double precision CENTER(1:3), center of sphere; undefined 
!    if A,B,C,D coplanar.
!
!    Output, double precision RADSQ, the square of radius of sphere; 
!    -1 if A,B,C,D coplanar.
!
!    Output, integer IN, contains following value if
!    INTEST is .TRUE.:
...
!
!    Input, integer IND(1:K+1), the indices in VCL of K-D vertices
!    of simplex.
!
!    Input, double precision VCL(1:K,1:*), K-D vertex coordinate list.
!
!    Input, double precision PT(1:K), the point for which sphere test
!    is applied (referenced iff INTEST is TRUE).
!
!    Output, double precision CENTER(1:K), the center of circumsphere; 
...
!
!    Input, double precision NRMLC(1:4), the unit normal vector of cut 
!    plane plus right hand side constant term of plane equation.
!
!    Input, double precision ANGACC, the minimum acceptable dihedral angle
!    in radians produced by a cut face.
!
!    Input, double precision DTOL, the absolute tolerance to determine
...
!
!    Input/output, double precision VCL(1:3,1:NVC), the vertex coordinate list.
!
!    Input, integer FACEP(1:3,1:*), the face pointer list.
!
!    Input, double precision NRML(1:3,1:*), the unit normal vectors for faces.
!
!    Input, integer FVL(1:6,1:*), the face vertex list.
!
!    Input, double precision EANG(1:*), the edge angles.
!
!    Input/output, integer NEDGC, the number of edges which 
!    intersect cut plane.
!
...
!
!    Input, double precision CDANG(1), the dihedral angle at starting edge
...
!
!    Output, double precision CDANG(1:NCE), the dihedral angles created by
...
!
!    Input, double precision RDACC, the minimum acceptable relative 
...
!    face type set to 0.
!
!    Input/output, double precision NRML(1:3,1:NFACE), the unit normal 
...
!    routine DSPHDC.
!
!    Input/output, double precision EANG(1:NVERT), the angles at edges 
...
!  Parameters:
!
!    Input, double precision ASPC2D, the angle spacing parameter in radians
!    used in controlling vertices to be considered as an endpoint of a
!    separator.
!
!    Input, double precision ATOL2D, the angle tolerance parameter in 
...
!  Parameters:
!
!    Input, double precision X0, Y0, X1, Y1, X2, Y2, X3, Y3,
!    vertex coordinates.
!
...
!
!    Output, double precision DIAMSQ, the square of diameter.
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!
!    Input, double precision VCL(1:3,1:NVRT), the vertex coordinate list.
!
...
!
!    Input, double precision P(1:K), Q(1:K), two points.
!
...
!
!    Input, double precision EANG(1:*), angles at edges common to 2 faces 
...
!
!    Output, double precision CNRML(1:3,1:NCFACE), the unit outward 
!    normals of faces.
!
!    Output, integer CFVL(1:5,1:NCVERT), face vertex list; see routine DSCPH.
!
!    Output, double precision CEANG(1:NCVERT), the angles at edges common
...
!
!    Input, double precision VCL(1:3,1:NVC), the vertex coordinate list.
!
...
!
!    Output, double precision NRML(1:3,1:NFACE), the unit outward normals
!    of polyhedron faces.
!
...
!
!    Input, double precision A(1:K,1:*), see routine DHPSRT.
!
...
!
!    Input, double precision VCL(1:2,1:NVC), vertex coordinates of boundary
...
!
!    Input, double precision VCL(1:2,1:NVC), the vertex coordinate list.
!
!    Input, integer HVL(1:NPOLG), the head vertex list.
!
!    Input, integer PVL(1:4,1:*), the polygon vertex list.
!
!    Input, double precision IANG(1:*), the interior angles.
!
...
!
!    Input, double precision NRML(1:3,1:NFACE), the unit normal vectors
!    for faces.
!
!    Input, integer FVL(1:6,1:NVERT), the face vertex list.
!
!    Input, double precision EANG(1:NVERT), the edge angles.
!
...
!
!    Output, double precision WID(1:NPOLH), the width of convex polyhedra.
!
!    Output, double precision FACVAL(1:NFACE), the value associated with 
!    each face of decomposition.
!
!    Output, double precision EDGVAL(1:NVERT), the value associated with 
!    each edge of decomposition.
!
!    Output, double precision VRTVAL(1:NVC), the value associated with 
...
!
!    Input, double precision VCL(1:2,1:NVC), the vertex coordinates of 
...
!
!    Output, double precision NRML(1:3,1:NFACE), the normals at faces;
...
!
!    Output, double precision EANG(1:NVERT), the angles at edges common 
...
!  Parameters:
!
!    Input, double precision ASPC2D, angle spacing parameter in radians
!    used in controlling vertices to be considered as an endpoint of a
!    separator.
!
!    Input, double precision ATOL2D, angle tolerance parameter in radians
...
!    separator.
!
!    Input, double precision ATOL2D, the angle tolerance parameter in radians
!    used in accepting separator to resolve a hole on a face.
!
!    Input, double precision ANGACC, the minimum acceptable dihedral angle in
!    radians produced by a cut face.
!
!    Input, double precision RDACC, the minimum acceptable relative distance
...
!
!    Input/output, double precision NRML(1:3,1:NFACE), the unit normal vectors 
...
!
!    Input, double precision VCL(1:3,NVC+1:NVC+NVCH), the vertex coordinate
!    list for hole.
!
...
!    Input, integer FC_MAX, the maximum size available for FC array.
!
!    Input, double precision VCL(1:K,1:*), the vertex coordinate list.
!
...
!
!    Input, double precision VCL(1:2,1:*), coordinates of 2D vertices.
!
...
!
!    Input, double precision VCL(1:3,1:NPT), the vertex coordinates.
...
!
!    Workspace, double precision WK(1:K*K+2*K+1).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!    enough.
!
!    Input, double precision VCL(1:2,1:*), the coordinates of 2D vertices.
!
...
!  Parameters:
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), the vertices 
!    of the tetrahedron.
!
!    Output, double precision EMNRTH, the mean ratio of the tetrahedron.
!
  implicit none
...
!
!    Input, external double precision UMDF(X,Y), the user-supplied mdf 
!    with d.p arguments.
!
...
!
!    Input, double precision DMIN, the parameter used to determine if 
...
!
!    Input/output, double precision VCL(1:2,1:NVC), the vertex coordinate 
!    list.
!
...
!    in decomposition.
!
!    Output, double precision PSI(1:NPOLG), the smoothed mean mdf values 
!    in the convex polygons.
!
!    Output, double precision H(1:NPOLG), the triangle size for convex 
!    polygons.
!
!    Workspace, integer IWK(1:MAXIW).
!
...
!    Input, logical HFLAG, TRUE if heuristic mdf, FALSE if user-supplied mdf.
!
!    Input, external double precision UMDF(X,Y,Z), d.p user-supplied mdf with 
!    d.p arguments.
!
!    Input, double precision KAPPA, the mesh smoothness parameter in 
...
!    in radians produced by cut faces.
!
!    Input, double precision ANGEDG, the angle parameter in radians used to 
...
!
!    Input/output, double precision VCL(1:3,1:NVC), the vertex coordinate 
!    list.
!
!    Input/output, integer FACEP(1:3,1:NFACE), the face pointer list: 
!    row 1 is head pointer, rows 2 and 3 are signed polyhedron indices.
...
!
!    Output, double precision VOL(1:NPOLH), the volume of convex polyhedra
!    in decomposition.
!
!    Output, double precision PSI(1:NPOLH), the mean mdf values in the 
!    convex polyhedra.
!
!    Output, double precision H(1:NPOLH), the tetrahedron size for
!    convex polyhedra.
!
!    Workspace, integer IWK(1:MAXIW).
!
...
!
!    Input, double precision MINBEF, the (min tetra measure of an existing
!    tetra of swap) + TOL.
!
...
!  Parameters:
!
!    Input, double precision ANGAC1, angle tolerance parameter used for
...
!    consecutive vertices is assumed to be < 180 degrees.
!
!    Input, double precision X(1:*), Y(1:*), IANG(1:*), integer LINK(1:*), 
...
!    link.
!
!    Output, double precision ANGSEP, the minimum of the 4 or 7 angles at 
!    the boundary resulting from 1 or 2 separators, respectively.
!
!    Output, integer I1, I2, the indices of endpoints of separators in XC, 
!    YC arrays; I2 = -1 if there is only one separator, else I1 < I2.
!
!    Workspace, double precision WKANG(0:NV), working array for angles.
!
  implicit none
...
!  Parameters:
!
!    Input, double precision XH, YH, the coordinates of hole vertex.
!
!    Input, integer NVRT, (number of vertices) - 1.
!
!    Input, double precision XC(0:NVRT), YC(0:NVRT), the vertex coordinates
...
!
!    Output, double precision ANGSEP, the minimum of 4 angles at boundary
...
!
!    Workspace, double precision MAT(1:K,1:K), matrix used for 
!    determining rank.
!
...
!
!    Input, double precision VCL(1:2,1:*), vertex coordinate list.
!
...
!
!    Input, double precision VCL(1:3,1:*), vertex coordinate list.
!
...
!    convex polygon.
!
!    Input, double precision double precision XC(0:NVRT), YC(0:NVRT), 
...
!
!    Input, double precision double precision COSTH, SINTH; COS(THETA),
...
!
!    Input/output, double precision IANG(1:NVERT), interior angles.
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!    Input/output, integer FVL(1:6,1:NVERT), the face vertex list.
!
!    Input/output, double precision EANG(1:NVERT), the edge angles.
!
...
!    Input, integer P, the polyhedron index.
!
!    Input, double precision NRMLC(1:3), the unit normal vector of cut plane.
!
...
!    index of new edge.
!
!    Input, double precision CDANG(1:NCE), dihedral angles created by edges
...
!  Parameters:
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), 4 vertices of 
!    tetrahedron.
!
!    Output, double precision CENTER(1:3), the center of insphere; undefined 
!    if A,B,C,D coplanar.
!
!    Output, double precision RAD, the radius of insphere; 0 if A,B,C,D 
!    coplanar.
!
  implicit none
...
!    Input, integer MAXPV, the maximum size available for PVL array.
!
!    Input/output, double precision VCL(1:2,1:NVC), vertex coordinate list.
!
!    Input/output, integer PVL(1:4,1:NVERT), polygon vertex list.
!
!    Input/output, double precision IANG(1:NVERT), the polygon interior angles.
!
...
!    Input, integer MAXFV, the maximum size available for FVL, EANG arrays.
!
!    Input, double precision VCL(1:3,1:NVC+1), the vertex coordinate list;
...
!
!    Input/output, double precision SVCL(1:3,1:NSVC), the vertex 
!    coordinate list.
!
...
!    realizing diameter.
!
!    Input, double precision H, the mesh spacing.
!
!    Input, integer MAXVC, the maximum size available for VCL array.
!
!    Input, integer MAXWK, the maximum size available for WK array; should 
...
!
!    Output, double precision VCL(1:3,1:NVC), the vertex coordinate list.
!
!    Workspace, double precision WK(1:MAXWK).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
...
!
!    Input, double precision CTRX, CTRY, the coordinates of the centroid
!    before translation.
!
...
!    with d.p arguments.
!
!    Input, double precision WSQ, square of width of original polygon 
!    of decomposition.
!
!    Input, integer NEV, IFV, LISTEV(1:NEV), the output from routine PRMDF2.
!
!    Input, integer IVRT(1:*), double precision EDGVAL(1:*), VRTVAL(1:*),
...
!
!    Output, double precision MDFTR(0:NVRT-1), mean mdf value in each 
...
!    Input, logical HFLAG, TRUE if heuristic mdf, FALSE if user-supplied mdf.
!
!    Input, external double precision UMDF(X,Y,Z), the user-supplied mdf 
!    with d.p arguments.
!
!    Input, integer HEADP, the head pointer to face of PFL for convex
!    polyhedron.
!
!    Input, double precision WIDP, the width of original polyhedron of
!    decomposition.
!
!    Input, integer NFCEV, NEDEV, NVREV, LISTEV(1:NFCEV+NEDEV+NVREV),
!    INFOEV(1:4,1:NFCEV+NEDEV), the output from routine PRMDF3.
!
!    Input, integer IVRT(1:*), double precision FACVAL(1:*), EDGVAL(1:*),
...
!    polyhedron.
!
!    Input, double precision CNTR(1:3), the weighted centroid of polyhedron.
!
...
!
!    Output, double precision MDFINT, the integral of mdf in polyhedron.
!
!    Output, double precision MEAN, the mean mdf value in polyhedron.
!
!    Output, double precision STDV, the standard deviation of mdf in
!    polyhedron.
!
!    Output, double precision VOLP, the volume of polyhedron.
!
!    Output, integer INDF(1:NF), the indices in FACEP of faces of polyhedron.
!
!    Output, double precision MEANF(1:NF), the mean mdf value associated 
!    with faces of polyhedron.
!
!    Output, double precision STDVF(1:NF), the standard deviation of 
...
!
!    Input, double precision ANGSPC, the angle spacing parameter used 
...
!  Parameters:
!
!    Input, double precision XU, YU, XV1, YV1, XV2, YV2, vertex coordinates;
...
!
!    Input, double precision DV, signed distance (positive for left).
!
...
!  Parameters:
!
!    Input, double precision PT(1:3), a 3D point.
!
!    Input, integer N, the upper bound on vertex indices or size of VM.
!
!    Input, integer P, the size of the hash table.
!
...
!  Parameters:
!
!    Input/output, double precision A(LDA,N).  On input, the N by N matrix
...
!
!    Input, double precision TOL, the relative tolerance for detecting
!    singularity of A.
!
...
!
!    Input/output, double precision B(1:N).  On input, the right hand 
...
!
!    Input, double precision WSQ, the square of width of polygon 
!    containing (X,Y).
!
!    Input, integer NEV, IFV, LISTEV(1:NEV), output from routine PRMDF2.
!
!    Input, integer IVRT(1:*), double precision EDGVAL(1:*), 
...
!  Parameters:
!
!    Input, double precision X, Y, Z, the coordinates of 3D point.
!
!    Input, double precision WIDP, the width of polyhedron containing (X,Y,Z).
!
!    Input, integer NFCEV, NEDEV, NVREV, LISTEV(1:NFCEV+NEDEV+NVREV),
...
!    Input, double precision VCL(1:3,1:*), the vertex coordinate list
!
!    Output, double precision MDF3, the reciprocal of cube of length scale 
!    at (X,Y,Z).
!
  implicit none
...
!    with d.p arguments.
!
!    Input, double precision KAPPA, the mesh smoothness parameter in interval
!    [0.0,1.0].
!
!    Input, double precision ANGSPC, the angle spacing parameter in radians 
!    used to determine extra points as possible endpoints of separators.
...
!    the polygon vertex list and interior angles.
!
!    Input, integer IVRT(1:NVERT), XIVRT(1:NPOLG+1), double precision !    WIDSQ(1:NPOLG), EDGVAL(1:NVERT), VRTVAL(1:NVC), arrays output from 
!    routine DSMDF2;  if .NOT. HFLAG then only first two arrays exist.
!
!    Input/output, double precision AREA(1:NPOLG), the area of convex 
...
!    in radians produced by cut faces.
!
!    Input, double precision ANGEDG, the angle parameter in radians used 
...
!    XIFAC(1:NPOLH+1), double precision WID(1:NPOLH), FACVAL(1:NFACE),
...
!
!    Ouptut, double precision VOL(1:NPOLH), the volume of convex polyhedra
!    in decomposition.
!
!    Output, double precision PSI(1:NPOLH), the mean mdf values in the
!    convex polyhedra.
...
!
!    Input, double precision XC(0:NVRT), YC(0:NVRT), coordinates of polygon
...
!    K to PVL(SUCC,K).
!
!    Input, double precision IANG(1:*), interior angles.
!
...
!  Parameters:
!
!    Input, double precision A(1:3), B(1:3), C(1:3), D(1:3), E(1:3),
!    five 3D points.
!
...
!
!    Input, double precision VCL(1:K,1:*), the K-D vertex coordinate list.
!
!    Input, logical EFLAG, TRUE if PTA = PTB: used to determine whether 
!    PTA lies in hyperplane containing face, only PTA referenced.
!
!    Input, double precision PTA(1:K), PTB(1:K), the K-D points for which
!    test is applied.
!
!    Workspace, double precision MAT(1:K-1,1:K), the matrix used for 
!    solving system of homogeneous linear equations.
!
!    Workspace, double precision VEC(1:K), the vector used for 
!    hyperplane normal.
!
...
!
!    Input, double precision WSQ, the square of width of polygon IPOLY.
!
...
!    to XIVRT(IPOLY+1)-1.
!
!    Input, double precision EDGVAL(1:*), the value associated with each 
!    edge of decomposition.
!
!    Input, double precision VRTVAL(1:*), the value associated with each 
!    vertex of decomposition.
!
...
!
!    Input, double precision WIDP, the width of polyhedron IPOLH.
!
!    Input, integer NVC, the number of vertex coordinates in VCL array.
!
...
!
!    Input, double precision FACVAL(1:*), the value associated with each 
...
!
!    Input, double precision PT(1:DIM), the point for which in/out test
!    is applied.
!
...
!    of the tetrahedron.
!
!    Output, double precision RADRTH, the radius ratio of the tetrahedron.
!
  implicit none
...
!
!    Input, double precision SCALE(1:K), TRANS(1:K), scale and translation
...
!
!    Output, double precision A(1:K,1:N), an array of N uniform random 
!    K-D points.
!
  implicit none
...
!
!    Input, double precision ANGACC, the minimum acceptable dihedral 
...
!
!    Input/output, double precision VCL(1:3,1:NVC), the vertex 
!    coordinate list.
!
!    Input/output, integer FACEP(1:3,1:NFACE), the face pointer list.
!
!    Input/output, integer FACTYP(1:NFACE), the face types.
!
!    Input/output, double precision NRML(1:3,1:NFACE), the unit 
...
!    plane plus right hand side constant term of plane equation.
!
!    Input, double precision PT(1:3), the point (of hole) on cut plane.
!
!    Input, double precision DIR(1:3), the unit direction of ray on cut 
...
!
!    Output, double precision PT1(1:2), coordinates of 1st separator
!    endpoint if W1 < 0.
!
!    Output, double precision PT2(1:2), coordinates of 2nd separator 
!    endpoint if W2 < 0.
!
!    Workspace, integer IWK(1:MAXIW).
!
!    Wokspace, double precision WK(1:MAXWK).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!    Input, double precision XH, YH, the coordinates of hole vertex.
!
!    Input, double precision ASPC2D, the angle spacing parameter in 
...
!
!    Output, double precision XV, YV, the coordinates of second separator
!    endpoint if V < 0.
!
!    Workspace, integer IWK(1:MAXIW).
!
...
!
!    Input/output, double precision NRML(1:3,1:NFACE), unit outward normals
!    of faces.
!
!    Input/output, integer FVL(1:5,1:NVERT), face vertex list; see routine
...
!
!    Input, double precision EANG(1:NVERT), angles at edges common to 2 
...
!    is a new vertex.
!
!    Input, double precision XC(0:NVRT), YC(0:NVRT), the vertices of polygon
...
!    of the tetrahedron.
!
!    Output, double precision SANG(1:4), the four solid angles at A, B, 
!    C, D, respectively.  (Actually sin(angle/2)).
!
!    Output, double precision SANGMN, sin(min solid angle / 2) 
...
!    of tetrahedron.
!
!    Output, double precision SANG(1:4), the solid angles (+ PI) at 
!    A,B,C,D, respectively.
!
!    Output, double precision DANG(1:6), the dihedral angles at
...
!    Input, double precision CNTR(1:3), the weighted centroid of polyhedron.
!
!    Input, double precision NRMLC(1:4), the unit normal vector of cut plane
!    plus right hand side constant term of plane equation.
!
!    Input, double precision ANGACC, the minimum acceptable dihedral angle
!    in radians produced by a cut face.
!
!    Input, double precision MXCOS, the maximum cosine of angle allowed 
...
!    Input, integer MAXVC, the maximum size available for VCL array.
!
!    Input/output, double precision VCL(1:3,1:NVC+2), the vertex 
!    coordinate list.
!
!    Input, integer FACEP(1:3,1:*), the face pointer list.
!
...
!    below, else CEDGE(2,0) is not used.
!
!    Input, double precision CDANG(1), dihedral angle at starting edge
!    determined by cut plane in positive half-space.
!
...
!  Parameters:
!
!    Input, double precision ANGTOL, the angle tolerance parameter 
!    (in radians).
!
!    Input, integer NVRT, the number of vertices in polygon.
!
...
!  Parameters:
!
!    Input, double precision ANGTOL, angle tolerance parameter (in radians).
!
...
!    K to PVL(SUCC,K).
!
!    Input, double precision IANG(1:*), interior angle array
!
!    Output, integer I1, I2, indices in range 0 to NVRT-1 of best separator
...
!    Input, double precision CNTR(1:3), the weighted centroid of polyhedron.
!
!    Input, double precision MEAN, the mean MDF value in polyhedron.
!
!    Input, integer NF, the number of faces in polyhedron.
!
!    Input, integer INDF(1:NF), the indices in FACEP of faces of polyhedron.
!
!    Input, double precision MEANF(1:NF), the mean mdf value associated 
!    with faces of polyhedron.
!
!    Input, double precision ANGACC, the min acceptable dihedral angle 
!    in radians produced by a cut face.
!
!    Input, double precision MXCOS, the max cosine of angle allowed for 
!    angles subtended by new subedges with respect to centroid.
!
!    Input, double precision DTOL, the absolute tolerance to determine if 
...
!    Input, double precision EANG(1:*), the edge angles.
!
!    Output, double precision NRMLC(1:4), the unit normal vector of cut 
...
!
!    Output, integer CEDGE(1:2,0:NCE), double precision CDANG(1:NCE), the 
...
!
  ...
  integer facep(3,*)
  integer , parameter :: facn = 2
  double precision , parameter, dimension ( 3 ) :: fract = (/ &
...
!    with d.p arguments.
!
!    Input, double precision WIDP, the width of original polyhedron 
!    of decomposition.
!
!    Input, integer NFCEV, NEDEV, NVREV, LISTEV(1:NFCEV+NEDEV+NVREV),
!    INFOEV(1:4,1:NFCEV+NEDEV), output from routine PRMDF3.
!
!    Input, integer IVRT(1:*), double precision FACVAL(1:*), 
...
!    Input, integer MAXVC, the maximum size available for VCL array.
!
!    Input/output, double precision VCL(1:3,1:NVC).  On input, vertex
...
!
!    Output, integer CEDGE(1:2,0:NCE), double precision CDANG(1:NCE), 
!    information describing cut polygon as output by routine SEPFAC.
!
...
!
!    Workspace, double precision VAL(1:2*NE).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!    in radians produced by a cut face.
!
!    Input, double precision MXCOS, the maximum cosine of angle allowed for
...
!
!    Workspace, double precision WVC(1:3,1:NV).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!
!    Input, double precision PSI(1:*), key values for heap.
!
...
!
!    Output, double precision XS(0:NSHR), YS(0:NSHR), the coordinates 
...
!  Parameters:
!
!    Input, double precision SDIST, the shrink distance.
!
!    Input, integer NFACE, the number of faces in convex polyhedron.
!
!    Input, double precision VCL(1:3,1:*), the vertex coordinate list.
...
!
!    Input, double precision NRML(1:3,1:NFACE), the unit outward normals
!    of faces.
!
!    Input, integer FVL(1:5,1:*), the face vertex list; see routine DSCPH; 
!    FVL may contain some unused columns (with <= 0 LOC value).
!
!    Input, double precision EANG(1:*), the angles at edges common to 2
!    faces; EANG(I) corresponds to FVL(*,I).
!
...
!
!    Output, double precision SVCL(1:3,1:NSVC), SHVL(1:NFACE),
...
!
!    Workspace, double precision CENTER(1:K), the hypersphere center or
!    hyperplane normal.
!
!    Workspace, double precision MAT(1:K,1:K), the matrix used for solving
!    system of linear equations.
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
...
!
!    Workspace, double precision X(1:*), Y(1:*), integer LOCFV(1:*), 
...
!
!    Input, double precision X(1:N), array of N data points.
!
!    Input, double precision A, H, used for determining frequency 
!    intervals as above.
!
!    Input, integer NF, the number of frequency intervals - 1.
!
!    Output, double precision XMIN, XMAX, MEAN, STDV, the minimum, 
!    maximum, mean, standard deviation.
!
!    Output, double precision FREQ(0:NF), the relative frequencies 
...
!
!    Input, double precision ALPHA(1:K), -1.0, 1.0, or 0.0 to indicate 
...
!
!    Workspace, double precision ALPHA(1:K+1), the barycentric coordinates 
...
!    of the tetrahedron.
!
!    Workspace, double precision S(1:4), needed for SANGMN routine
!
!    Output, double precision TETMU, depending on CRIT, contains:
!    1, SANGMN(ABCD) * SF, 
!    2, RADRTH(ABCD),
!    3, EMNRTH(ABCD).
!
...
!    each polyhedron.
!
!    Input, double precision VOL(1:NPOLH), the volume of convex polyhedra 
!    in decomposition.
!
!    Input/output, double precision PSI(1:NPOLH), the mean mdf values in 
!    the convex polyhedra.  On output, the values have been smoothed.
!
!    Output, double precision H(1:NPOLH), the tetrahedron size for convex
!    polyhedra.
!
...
!  Parameters:
!
!    Input, double precision H(1:NPOLH), the mesh spacings for NPOLH 
...
!  Parameters:
!
!    Input, double precision H(1:NPOLH), the mesh spacings for convex 
!    polyhedra in polyhedral decomposition data structure.
!
!    Input, double precision SHRF, the factor for shrinking polyhedron; 
...
!
!    Input, double precision EANG(1:NVERT), the angles at edges common 
...
!    Workspace, integer IWK(1:MAXIW).
!
!    Worksapce, double precision WK(1:MAXWK).
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!  Parameters:
!
!    Input, double precision X, Y, the coordinates of 2D point.
!
!    Output, double precision UMDF2, the mesh distribution function 
!    value at (X,Y).
!
  implicit none
...
!    Input, double precision X, Y, Z, the coordinates of 3D point
!
!    Output, double precision UMDF3, the mesh distribution function 
!    value at (X,Y,Z)
!
  implicit none
...
!
!    Output, double precision URAND, a pseudorandom value.
!
  implicit none
  ...
  integer , save :: m2 = 0
  integer , save :: mic = 0
  double precision , save :: s = 0.0E+00
  double precision urand
...
!  Parameters:
!
!    Input, double precision X, Y, the 2D point outside convex hull.
!
!    Input, double precision VCL(1:2,1:*), the coordinates of 2D vertices.
!
...
!  Parameters:
!
!    Input, double precision PT(1:3), the 3D point.
!
!    Input, double precision CTR(1:3), the 3D point in interior of
!    triangulation.
!
!    Input, double precision VCL(1:3,1:*), the vertex coordinate list.
!
...
!    Input, integer K, the dimension of triangulation.
!
!    Input, double precision PT(1:K), the K-D point.
!
!    Input, double precision CTR(1:K), the K-D point in interior of
!    triangulation.
!
!    Input, double precision VCL(1:K,1:*), the vertex coordinate list.
!
...
!  Parameters:
!
!    Input, double precision ANGSPC, the angle spacing parameter in 
...
!    Input, integer FVL(1:5,1:*), the face vertex list; see routine DSCPH.
!
!    Output, double precision VOLCPH, the volume of convex polyhedron.
!
  implicit none
...
!    of tetrahedron.
!
!    Output, double precision VOLTH, 6 * volume of tetrahedron.
!
  implicit none
...
!  Parameters:
!
!    Input/output, double precision XC, YC, see comments in routine VISPOL.
!
...
!  Parameters:
!
!    Input/output, double precision XC, YC, see comments in VISPOL.
!
...
!  Parameters:
!
!    Input, double precision X, Y, the 2D point.
!
...
!
!    Output, double precision WIDSQ, the square of width.
!
!    Output, integer IERR, error flag, which is zero unless an error occurred.
!
  implicit none
...
!
!    Output, double precision WID, the width of convex polyhedron.
!
...
!
!    Input, double precision XV1, YV1, XV2, YV2, XW1, YW1, XW2, YW2,
...
!
!    Input, double precision DV, DW, the signed distances (positive for
!    left).
!
!    Output, double precision XU, YU, the coordinates of the point of
...
!  Parameters:
!
!    Input, double precision AR, BR, DR, the half-plane equation is 
!    AR*X + BR*Y <= DR.
!
...
!    convex polygon.
!
!    Input/output, double precision SVCL(1:3,1:MAXSV), 
